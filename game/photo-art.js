...
function landscapeRoles(withStamp) {
  const g = grid(N, N, "sky");
  for (let y = 38; y < 52; y++) {
    for (let x = 0; x < N; x++) g[y][x] = "sea";
  }
  for (let x = 0; x < N; x++) {
    const sandY = x > 36 ? Math.max(45, Math.round(52 - (x - 36) / 3)) : 52;
    for (let y = sandY; y < N; y++) g[y][x] = "sand";
    set(g, x, sandY, "foam");
  }
  const cx = 48;
  const foot = 52;
  ...
  for (let y = top + 8; y < foot; y++) {
    ...
    for (let x = cx - half; x <= cx + half; x++) set(g, x, y, "white");
  }
  for (let y = top + 6; y < top + 9; y++) {
    for (let x = cx - 4; x < cx + 5; x++) set(g, x, y, "red");
  }
  for (let y = top + 2; y < top + 7; y++) {
    for (let x = cx - 2; x < cx + 3; x++) set(g, x, y, "white");
  }
  for (let i = 0; i < 6; i++) {
    const w = 7 - i;
  ...
  if (withStamp) {
    const mark = grid(N, N, 0);
    text(mark, 4, 6, "98 7", 1);
    for (let y = 0; y < N; y++) {
      for (let x = 0; x < N; x++) {
        if (mark[y][x] && g[y][x] === "sky") g[y][x] = "stamp";
      }
    }
  }
  return g;
